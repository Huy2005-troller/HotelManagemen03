...
namespace HotelManagement.DataAccess
{
    public class Repository : IRepository
    {
        ...
        public void deleteAllDichVuTrongOrder(string maOrderPhong)
        {
            ...
            context.OrderPhongDichVus.RemoveRange(oldItems);
            context.SaveChanges();
        }
        ...
        public bool addHoaDonDoHong(HoaDonDoHong hoaDonDoHong)
        {
            try
            {
                context.HoaDonDoHongs.Add(hoaDonDoHong);
                int check = context.SaveChanges();
                return check > 0;
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Lỗi khi thêm đồ hỏng: {ex.Message}");
                return false;
            }
        }
